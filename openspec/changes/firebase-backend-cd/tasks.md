...
- [ ] 1.1 Create `.github/workflows/firebase-backend-deploy.yml`, triggered
...
- [ ] 1.2 Ensure the temporary credentials file is removed in an `if: always()`
      cleanup step.

## 2. Terraform Cleanup

- [ ] 2.1 Remove `google_firebaserules_ruleset.firestore` and
      `google_firebaserules_release.firestore` from `terraform/firebase.tf`.
- [ ] 2.2 Run `terraform state rm google_firebaserules_ruleset.firestore` and
      `terraform state rm google_firebaserules_release.firestore`.
- [ ] 2.3 Run `terraform plan` and confirm it reports no unexpected changes.

## 3. Validation

- [ ] 3.1 Lint/sanity-check the new workflow YAML (e.g. via `act` dry run or
