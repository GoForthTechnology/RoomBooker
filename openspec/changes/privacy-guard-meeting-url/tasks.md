...
- [ ] 5.1 Add `functions/scripts/migrate_meeting_urls.js`: iterate every org's
  ...
  per document).
- [ ] 5.2 Add a short usage note (how to run it, against which project) as a
  comment header in the script.
- [ ] 5.3 Run the script against the Firestore emulator
  (`scripts/start_emulators.sh`) with seeded data and verify the
  before/after state.

## 6. Validation

- [ ] 6.1 Run `(cd packages/roombooker_core && flutter test)`.
- [ ] 6.2 Run `(cd packages/roombooker_portal && flutter test)`.
- [ ] 6.3 Run `flutter analyze` from the workspace root.
- [ ] 6.4 Manually verify in the Kiosk: a confirmed booking with a meeting
  link shows the "Join Meeting" button and launches correctly.
