...
- [ ] 1.1 Fix `avoid_print` (5) in `packages/roombooker_portal/lib/main.dart`
      (lines 22, 27, 29, 46, 50) — replace with the project's
      logging/Sentry conventions or remove debug scaffolding.
- [ ] 1.2 Fix `experimental_member_use` (1) in
...
- [ ] 1.3 Fix `unnecessary_import`/`unused_import` (2) in
...
- [ ] 1.4 Fix `deprecated_member_use` (1) in
...
- [ ] 2.1 Fix `depend_on_referenced_packages` (2) for
...
- [ ] 2.2 Fix `unnecessary_underscores` (2) in
