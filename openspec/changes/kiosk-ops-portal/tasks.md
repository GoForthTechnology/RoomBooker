...
- [ ] 1.1 Add `adminRevokeKioskGrant` to `functions/index.js`: verify
...
- [ ] 2.1 Add `KioskGrantRecord` class to
...
- [ ] 2.2 Add `listKioskGrants(orgID, roomID)` to `ProvisioningService`
      returning `Stream<List<KioskGrantRecord>>` by listening to
      `orgs/{orgID}/rooms/{roomID}/kiosk-grants`.
- [ ] 2.3 Add `adminRevokeKioskGrant(orgID, roomID)` to
      `ProvisioningService` that calls the new Cloud Function.
- [ ] 2.4 Export `KioskGrantRecord` from `roombooker_core.dart`.

## 3. Portal UI — Room list Kiosk status section

- [ ] 3.1 In `room_list_widget.dart`, replace the static "Provision
      Kiosk" `IconButton` with a `StreamBuilder<List<KioskGrantRecord>>`
      on `provisioningService.listKioskGrants(org.id!, room.id!)`.
- [ ] 3.2 When stream is empty: render the existing "Link Kiosk"
      icon button (no behaviour change).
- [ ] 3.3 When stream has exactly one grant: render a small info row
...
- [ ] 3.4 When stream has more than one grant: render "Multiple devices"
      warning text and a single "Revoke All" icon button.
- [ ] 3.5 "Revoke" / "Revoke All" button shows a confirmation
