...
- [ ] 1.1 In `packages/roombooker_core/lib/data/entities/request.dart`, add
...
- [ ] 1.3 Add/update a `roombooker_core` test verifying a `Request` without
...
- [ ] 2.1 Create `packages/roombooker_kiosk/lib/quick_book_panel.dart`
      with a stateless `QuickBookPanel` widget taking `List<Request>
      bookings`, `DateTime now`, and `void Function(Duration) onBook`.
- [ ] 2.2 Implement gap computation: find the first booking with
...
- [ ] 2.3 Render three buttons (15m/30m/60m) styled consistently with the
...
- [ ] 3.1 In `_KioskDashboardState.build` (`main.dart`), render
      `QuickBookPanel` only when `status == RoomStatus.available`, passing
      `bookings` and `now`.
- [ ] 3.2 Implement the `onBook(Duration)` callback: build a `Request`
...
- [ ] 3.3 Surface errors from `addBooking` (e.g. a `SnackBar`), consistent
      with other error handling in `_KioskDashboardState`.

## 4. Portal: display Kiosk attribution

- [ ] 4.1 In the Portal's request log/agenda rendering
...
- [ ] 4.2 Add/update a `roombooker_portal` widget test covering the
...
- [ ] 5.1 Add `packages/roombooker_kiosk/test/quick_book_panel_test.dart`
...
- [ ] 6.1 Run `(cd packages/roombooker_core && flutter test)` and confirm
      all tests pass.
- [ ] 6.2 Run `(cd packages/roombooker_kiosk && flutter test)` and confirm
      all tests pass.
- [ ] 6.3 Run `(cd packages/roombooker_portal && flutter test)` and
      confirm all tests pass.
- [ ] 6.4 Run `flutter analyze` from the repo root and confirm no new
      issues.
- [ ] 6.5 Run `openspec validate kiosk-instant-booking --strict` and
