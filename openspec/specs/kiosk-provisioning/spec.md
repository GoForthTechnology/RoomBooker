...
- **THEN** the Kiosk SHALL receive its assigned `roomID` and `orgID` and persist them to secure local storage.
...
To prevent accidental or unauthorized device resetting, the de-provisioning control MUST be located within a multi-step navigation flow or behind an informational layer.
