...
void main() {
  ...
  testWidgets('ConfirmedOneOffBookings displays only one-off bookings', (WidgetTester tester) async {
    ...
    when(() => mockBookingRepo.getRequestDetails(orgID, oneOffRequest.id!))
    ...
    // Repeating shouldn't even need details fetching if filtered out correctly by viewmodel but let's provide it to be safe/if logic fetches all then filters
     when(() => mockBookingRepo.getRequestDetails(orgID, repeatingRequest.id!))
        .thenAnswer((_) => Stream.value(requestDetails));
  ...
  testWidgets('ConfirmedRepeatingBookings displays only repeating bookings', (WidgetTester tester) async {
     when(() => mockBookingRepo.listRequests(
          orgID: any(named: 'orgID'),
          startTime: any(named: 'startTime'),
          endTime: any(named: 'endTime'),
